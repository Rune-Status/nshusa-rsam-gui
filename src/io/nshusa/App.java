...
import io.nshusa.util.MiscUtils;
		...
		try {
			...
			final FutureTask<Boolean> query = new FutureTask<>(() -> {
                ...
                try (BufferedReader in = new BufferedReader(
                    ...
                    if (!App.properties.getProperty("version").equals(version)) {
                        ...
                        if (result.isPresent()) {
                            if (result.get() == ButtonType.OK) {
                                MiscUtils.launchURL(App.properties.getProperty("creator_link"));
                                System.exit(1);
                            }
                        }
			...
			ex.printStackTrace();
			System.out.println("Failed checking for an update.");
		}
